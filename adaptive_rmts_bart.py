...
class AdaptiveRMTSBartForConditionalGeneration(BartForConditionalGeneration):
    def __init__(
        self,
        config,
        num_traits=11,
    ...
    def _apply_score_head(self, feature, prompt_ids):
        ...
        for prompt_id in unique_prompt_ids.tolist():
            ...
            prompt_scores[prompt_mask] = self.prompt_score_heads[prompt_id](
                feature[prompt_mask]
            )
    ...
    def encode_multi_view(
        ...
        gate_feature = torch.cat(
            [essay_pool, gpt_pool, llama_pool, prompt_embed],
            dim=-1,
        )
        ...
        if self.fixed_equal_gate:
            ...
            if keep_mask is not None:
                ...
                gate_scores = masked_scores / masked_scores.sum(
                    dim=-1,
                    keepdim=True,
                ).clamp_min(1e-8)
        else:
        ...
        gpt_feature = self._build_view_feature(
            essay_pool,
            gpt_pool,
            prompt_embed,
        )

        llama_feature = self._build_view_feature(
            essay_pool,
            llama_pool,
            prompt_embed,
        )

        fused_feature = self._build_view_feature(
            essay_pool,
            fused_pool,
            prompt_embed,
        )
