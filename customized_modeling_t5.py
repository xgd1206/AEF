...
class T5Block(nn.Module):
    ...
    def forward(
        ...
        if use_cache:
            outputs = outputs + \
                (present_key_value_state,) + attention_outputs
        else:
...
class T5PreTrainedModel(PreTrainedModel):
    ...
    def _init_weights(self, module):
        ...
        if isinstance(module, T5LayerNorm):
            ...
            if hasattr(
                    module.wi_0,
                    "bias") and module.wi_0.bias is not None:
            ...
            if hasattr(
                    module.wi_1,
                    "bias") and module.wi_1.bias is not None:
    ...
    def forward(
        ...
        for i, (layer_module, past_key_value) in enumerate(
            ...
            if self.model_parallel:
                ...
                # Ensure that attention_mask is always on the same device
                # as hidden_states
                ...
                if encoder_decoder_position_bias is not None:
                    encoder_decoder_position_bias = encoder_decoder_position_bias.to(
                        hidden_states.device)
                if layer_head_mask is not None:
                    layer_head_mask = layer_head_mask.to(
                        hidden_states.device)
            ...
            # hidden-states, key-value-states, (self-attention position
            # bias), (self-attention weights), (cross-attention position
            # bias), (cross-attention weights)
            ...
            # Model Parallel: If it's the last layer for that device, put
            # things on the next device
            if self.model_parallel:
                for k, v in self.device_map.items():
...
class CustomizedT5ForConditionalGeneration(T5PreTrainedModel):
    ...
    def __init__(self, config: T5Config):
        ...
        self.rationale_split_index = 512
        self.rationale_length = 512
    ...
    def init_Qformer(
        ...
        encoder_config = BertConfig.from_pretrained(local_bert_path)
        ...
        Qformer = BertLMHeadModel.from_pretrained(
            local_bert_path, config=encoder_config)
    ...
    def forward(
        ...
        if encoder_outputs is None:
            ...
            if self.use_rationale:
                split_index = self.rationale_split_index
                rationale_length = self.rationale_length
                input_ids2 = input_ids[:, :split_index]
                if attention_mask is None:
                    essay_attention_mask = torch.ones(
                        input_ids2.size(),
                        dtype=torch.long).to(self.device)
                else:
                    essay_attention_mask = attention_mask[:, :split_index]
                encoder_outputs = self.encoder(
                    input_ids=input_ids2, attention_mask=essay_attention_mask)
                hidden_states = encoder_outputs[0]

                criteria_ids = input_ids[:, split_index:split_index + rationale_length]
                if attention_mask is None:
                    criteria_attention_mask = torch.ones(
                        criteria_ids.size(),
                        dtype=torch.long).to(self.device)
                else:
                    criteria_attention_mask = attention_mask[:, split_index:split_index + rationale_length]
                criteria_encoder_outputs = self.encoder(
                    input_ids=criteria_ids, attention_mask=criteria_attention_mask)

                hidden_states = self.proj(
                    torch.concat(
                        [
                            encoder_outputs[0],
                            criteria_encoder_outputs[0]],
                        dim=1).permute(
                        0,
                        2,
                        1)).permute(
        ...
        elif return_dict and isinstance(encoder_outputs, BaseModelOutput):
    ...
    def _reorder_cache(self, past, beam_idx):
        ...
        for layer_past_states in past:
            ...
            for layer_past_state in layer_past_states:
                # need to set correct `past` for each of the four key /
                # value states
            ...
            assert len(reordered_layer_past_states) == len(
                layer_past_states)
...
class T5EncoderModel(T5PreTrainedModel):
    ...
    def _prune_heads(self, heads_to_prune):
        ...
        """
        for layer, heads in heads_to_prune.items():
            self.encoder.block[layer].layer[0].SelfAttention.prune_heads(
                heads)
...
warnings.filterwarnings(
    "ignore",
    message="Some weights of BertLMHeadModel were not initialized*")
    ...
    def __init__(self, config):
        ...
        self.register_buffer(
            "position_ids", torch.arange(
                config.max_position_embeddings).expand(
                (1, -1)))
    ...
    def forward(
        ...
        if position_ids is None:
            position_ids = self.position_ids[:,
                                             past_key_values_length: seq_length + past_key_values_length].clone()
...
class BertLayer(nn.Module):
    ...
    def forward(
        ...
        # decoder uni-directional self-attention cached key/values tuple is
        # at positions 1,2
    ...
    def feed_forward_chunk_query(self, attention_output):
        ...
        layer_output = self.output_query(
    ...
    def get_extended_attention_mask(
        ...
        if attention_mask.dim() == 3:
            ...
            if is_decoder:
                ...
                # causal and attention masks must have same type with
                # pytorch version < 1.3
...
class BertLMHeadModel(BertPreTrainedModel):
    ...
    def forward(
        ...
        if labels is not None:
            ...
            if reduction == "none":
                lm_loss = lm_loss.view(
                    prediction_scores.size(0), -1).sum(1)
